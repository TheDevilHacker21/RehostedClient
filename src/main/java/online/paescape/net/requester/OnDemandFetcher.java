...
import online.paescape.collection.Queue;
...
public class OnDemandFetcher extends OnDemandFetcherParent
    ...
    private final void readData() {
        try {
            ...
            if (expectedSize == 0 && readAbleBytes >= 10) {
                ...
                current = null;
                for (OnDemandRequest OnDemandRequest = (OnDemandRequest) requested.reverseGetFirst(); OnDemandRequest != null; OnDemandRequest = (OnDemandRequest) requested.reverseGetNext()) {
    ...
    public final void start(CacheArchive streamLoader, Client client) {/*
        ...
        landscapeIds[2] = 1986;
        objectMapIds[2] = 1987;
        data = new Stream(FileOperations.ReadFile(Signlink.findcachedir() + "map_index"));
        int osrsMaps = data.readUnsignedWord();

        System.out.println("OSRS Map amount: " + osrsMaps);

        regionIdsOSRS = new int[osrsMaps];
        landscapeMapIdsOSRS = new int[osrsMaps];
        objectMapIdsOSRS = new int[osrsMaps];
        for (int i2 = 0; i2 < osrsMaps; i2++) {
            regionIdsOSRS[i2] = data.readUnsignedWord();
            landscapeMapIdsOSRS[i2] = data.readUnsignedWord();
            objectMapIdsOSRS[i2] = data.readUnsignedWord();
        }
    ...
    public final void requestFileData(int type, int id) {
        ...
        synchronized (queue) {
            ...
            synchronized (fetchFromCacheList) {
                fetchFromCacheList.insertHead(OnDemandRequest_2);
            }
    ...
    public final void run() {
        try {
            while (running) {
                ...
                if (!flag) {
                    for (OnDemandRequest OnDemandRequest_1 = (OnDemandRequest) requested.reverseGetFirst(); OnDemandRequest_1 != null; OnDemandRequest_1 = (OnDemandRequest) requested.reverseGetNext()) {
                        flag = true;
    ...
    public final void insertExtraFilesRequest(int fileId, int idx) {
        ...
        synchronized (nodeList_1) {
            nodeList_1.insertHead(OnDemandRequest);
        }
    }
    ...
    public final OnDemandRequest getNextNode() {
        ...
        synchronized (zippedNodes) {
            OnDemandRequest = (OnDemandRequest) zippedNodes.popHead();
        }
        ...
        try {
            ...
            _ex.printStackTrace();
            System.out.println("File: " + OnDemandRequest.id + " DataType " + OnDemandRequest.dataType);
    ...
    public final int getMapIdForRegions(int landscapeOrObject, int regionY, int regionX) {
        ...
        for (int j1 = 0; j1 < (osrs ? regionIdsOSRS.length : regionIds.length); j1++) {
            if ((osrs ? regionIdsOSRS[j1] : regionIds[j1]) == regionId) {
                if (landscapeOrObject == 0) {
                    if (osrs && landscapeMapIdsOSRS[j1] == -1 || !osrs && landscapeIds[j1] == -1) {
                        return -1;
                    }

                    return (osrs ? landscapeMapIdsOSRS[j1] : landscapeIds[j1]);
                } else {
                    if (osrs && objectMapIdsOSRS[j1] == -1 || !osrs && objectMapIds[j1] == -1) {
                        return -1;
                    }

                    return (osrs ? objectMapIdsOSRS[j1] : objectMapIds[j1]);
                }
            }
        }
        return -1;
    ...
    private final void handleFailed() {
        ...
        while (extraFileCount < 10) {
            ...
            OnDemandRequest_1 = (OnDemandRequest) receivedFilesList.popHead();
            ...
            try {
                ...
                requested.insertHead(OnDemandRequest_1);
                extraFileCount++;
                closeRequest(OnDemandRequest_1);
                waiting = true;
            } catch (Exception e) {
    ...
    /**
     * Gets the nearby regions
     *
     * @param regionId The region id
     * @return returns the nearest regions
     */
    ...
    public void clearNearRegions(int regionId) {
        ...
        for (int region : regions) {
            for (int j1 = 0; j1 < regionIdsOSRS.length; j1++) {
                if (regionIdsOSRS[j1] == region) {
                    landscapeMapIdsOSRS[j1] = -1;
                    objectMapIdsOSRS[j1] = -1;
                }
            }
        }
    }
    ...
    private final void checkReceived() {
        OnDemandRequest OnDemandRequest;
        synchronized (fetchFromCacheList) {
            OnDemandRequest = (OnDemandRequest) fetchFromCacheList.popHead();
        }
        while (OnDemandRequest != null) {
            ...
            synchronized (fetchFromCacheList) {
                if (data == null) {
                    receivedFilesList.insertHead(OnDemandRequest);
                } else {
                    OnDemandRequest.buffer = data;
                    synchronized (zippedNodes) {
                        zippedNodes.insertHead(OnDemandRequest);
                    }
                }
                OnDemandRequest = (OnDemandRequest) fetchFromCacheList.popHead();
            }
    ...
    public final void get(int type, int idx) {
        requestFileData(type, idx);
    }
