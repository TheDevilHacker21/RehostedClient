...
public class Sprite extends DrawingArea implements RSSpritePixels {
    ...
    public Sprite(String var1, int dummy) {
        ...
        try {
            Image var2 = ResourceLoader.loadImage("background.png");
    ...
    public Sprite(CacheArchive streamLoader, String name, int idx) {
        this.name = name;
        this.idx = idx;
        Stream stream = new Stream(streamLoader.getDataForName(name + ".dat"));
        ...
        if (i1 == 0) {
            ...
            if ((name.equals("mod_icons") && (idx == 7 || idx == 8 || idx == 9)))
                setTransparency(255, 255, 255);
            else
                setTransparency(255, 0, 255);
            return;
